...
class ServiceGenerator {
  ...
  public genFile() {
    ...
    try {
      ...
      glob
        .sync(`${finalPath}/**/*`)
        .filter((ele: string | string[]) => !ele.includes('_deperated'))
        .forEach((ele: any) => {
          rimraf.sync(ele);
        });
    } catch (error) {
  ...
  public getResponseTP(responses: ResponsesObject = {}) {
    const response: ResponseObject | undefined =
      responses && this.resolveRefObject(responses['200'] || responses.default);
    ...
    // find ref to components hierarchy to get the real type
  ...
  public getInterfaceTP() {
    ...
    const data =
      ...
      [components.schemas].map((defines) => {
        ...
        // remove duplicated definitions by typeName
        // const uniqueDefines = Object.entries(defines).reduce((acc, [typeName, schema]) => {
        //   if (!acc[typeName]) {
        //     acc[typeName] = schema;
        //   }
        //   return acc;
      ...
      ['get', 'put', 'post', 'delete', 'patch'].forEach((method) => {
        ...
        if (operationObject.parameters) {
          operationObject.parameters.forEach((parameter: any) => {
            let param = parameter;
            // find ref parameter in components
            if (parameter.$ref) {
              const ref = parameter.$ref.split('/').pop();
              const refParameter = this.openAPIData.components.parameters[ref];
              if (refParameter) {
                param = refParameter;
              }
            }
