  ...
  sampleFromSchema = (schema: any, propsName?: string[], level?: number) => {
    ...
    if (type === 'object') {
      ...
      for (const name in props) {
        obj[name] = this.sampleFromSchema(props[name], [...(propsName || []), name], level ? level + 1 : 1);
      }
      ...
      if (additionalProperties) {
        ...
        const additionalPropVal = this.sampleFromSchema(additionalProps, propsName, level ? level + 1 : 1);
    ...
    if (type === 'array') {
      const item = this.sampleFromSchema(items, propsName, level ? level + 1 : 1);
